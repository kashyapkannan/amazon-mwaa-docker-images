...
generate_fernet_key() {
    ...
    chmod +x temporary-pip-install
    ./temporary-pip-install cryptography >/dev/null 2>&1
    
    # Generate the key and format as JSON
    KEY=$(python3 -c 'from cryptography.fernet import Fernet; import json; key = Fernet.generate_key().decode(); print(json.dumps({"FernetKey": key}))')
